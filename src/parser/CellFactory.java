...
public class CellFactory {
    ...
    public CellFactory (Element gridConfigurationTag, String simulationType) {
        myGridConfigurationTag = gridConfigurationTag;
        mySimulationType = simulationType;
        myResource =
                ResourceBundle.getBundle(InitializeSimulation.DEFAULT_RESOURCE_PACKAGE +
                                         this.getClass().getSimpleName());
        myGridBounds = new int[] { getDimension("breadth", myGridConfigurationTag),
    ...
    private int getDimension (String direction, Element gridConfigurationTag) {
        Element sizeTag =
                (Element) ((Element) gridConfigurationTag.getElementsByTagName("gridProperties")
                        .item(0)).getElementsByTagName("size").item(0);
        return Integer.parseInt(sizeTag.getAttributes().getNamedItem(direction).getNodeValue());
    }

    /**
     * Method getCellParser. Creates a Cell Parser based on whether the initial configuration of the
     * cells is random or location based
     *
     * @return CellParser
     */
    private CellParser getCellParser () {
        String typeOfInitialConfig =
                ((Element) myGridConfigurationTag.getElementsByTagName("cellConfiguration").item(0))
                        .getAttributes().getNamedItem("type").getNodeValue();
        String typeOfCellConfigClassName = myResource.getString(typeOfInitialConfig);
        try {
            Constructor<?> c =
                    Class.forName(typeOfCellConfigClassName)
                            .getConstructor(Element.class, String.class, ResourceBundle.class,
                                            int[].class);
            return (CellParser) c.newInstance(myGridConfigurationTag, mySimulationType, myResource,
                                              myGridBounds);
        }
        catch (Exception e) {
            e.printStackTrace();
            throw new ParserException("Couldn't find Cell Parser for " + typeOfInitialConfig,
                                      typeOfCellConfigClassName);
        }
    }
}
